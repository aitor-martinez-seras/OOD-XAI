...
def create_cluster_space_for_each_class_agglomerative_clustering(ssim_distance_matrix_per_class, class_names: list,
                                                                 in_dataset: str, model_arch: str, args: dict):
    '''
    ...
    silhouette_path_pdf = os.path.join(FIGURES_DIR_NAME, f'silhouetteScores_{in_dataset}_{model_arch}'
    ...
    dendrogram_path_pdf = os.path.join(FIGURES_DIR_NAME, f'DendrogramPerClass_{in_dataset}_{model_arch}'
                                                         f'_{args["load_or_train"]}_seed{args["seed"]}.pdf')
